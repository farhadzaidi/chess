...
def draw_moves(from_index, valid_moves):
	for index in valid_moves.get(from_index, []):
...
mg = MoveGenerator()
valid_moves = mg.generate_pseudo_legal_moves(b.board, b.castling_rights)
		...
		if event.type == pygame.MOUSEBUTTONDOWN:
			...
			if 0 <= row <= 7 and 0 <= col <= 7:
				if selected is None:
					...
					if to_index in valid_moves.get(from_index, []):
						move = Move(from_index, to_index, b.board[from_index])
						b.make_move(move)
						valid_moves = mg.generate_pseudo_legal_moves(b.board, b.castling_rights,
							b.moves[-1])
